...
RED.view = (function () {
	...
	RED.keyboard.add(/* o */ 79, {ctrl: true}, function () {
		arrangeAll();
		d3.event.preventDefault();
	});
					...
					if (nn.defaults.hasOwnProperty(prop)) {
						...
						if (entry) {
							...
							if (entry.hasOwnProperty("input")) {
								...
								if (inputType === "text-array") {
									var arr = nn.defaults[prop].value;
									if ($.isArray(arr)) {
										for (var idx = 0; idx < arr.length; idx++) {
											currInput += text.replace(/###prop###/g, prop + "-" + idx).replace(/###label###/g, label + " " + idx);
										}
									}
								}
